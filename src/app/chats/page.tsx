...
const dummyMessages: Message[] = [
  ...
  { id: 3, text: "I'm good, thanks. Wanna catch up later?", sender: 'other', timestamp: '10:05 AM' },
  { id: 4, text: 'Sure, let me know when.', sender: 'me', timestamp: '10:06 AM' },
...
export default function Chats() {
  ...
  const sendMessage = () => {
    ...
    setMessages(prev => [
      ...prev,
      { id: prev.length + 1, text: newText.trim(), sender: 'me', timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }
    ]);
    setNewText('');
  };
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      <div ref={containerRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        {messages.map(msg => (
          <div
            key={msg.id}
            className={`max-w-[70%] px-4 py-2 rounded-lg relative ${msg.sender === 'me' ? 'bg-green-500 text-white ml-auto' : 'bg-white text-gray-800 mr-auto'}`}
          >
            <p>{msg.text}</p>
