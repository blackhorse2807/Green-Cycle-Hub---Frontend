...
export default function ProductsDisplay({
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      const response = await authFetch(`${backend_url}/products/${productToDelete}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      });
      if (!response.ok) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            >
              ...
              <div className="p-4" onClick={() => onView(product)}>
                ...
                <div className="flex justify-end space-x-2 mt-2">
                  ...
                  >
                    {product.is_approved === 1 ? "Approved" : product.is_approved === 0 ? "Pending" : "Rejected"}
                  </span>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-xl shadow-lg p-8 max-w-sm w-full text-center">
            <div className="text-red-600 text-2xl font-bold mb-2">Confirm Delete</div>
            <div className="mb-4">
