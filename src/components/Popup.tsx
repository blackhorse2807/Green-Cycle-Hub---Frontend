...
const Popup: React.FC<PopupProps> = ({
  ...
  const getButtonClasses = (color: string) => {
    const baseClasses = "px-6 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 shadow-sm";
    switch (color) {
      case "red":
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={`flex justify-center gap-3 pt-2 ${!showLeftButton ? 'mt-4' : ''}`}>
          {showLeftButton && (
            <button
              onClick={onleft}
