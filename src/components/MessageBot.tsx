...
export default function Chatbot() {
  ...
  const nextQuestion = () => {
    ...
    if (next < questions.length) {
      setQIndex(next);
      setMessages(m => [...m, { type: 'bot', content: questions[next].text }]);
    }
  };
  ...
  const handleOption = async (opt: Option) => {
    ...
    if (q.id === 'initial' && opt.value === 'no') {
      setMessages(m => [...m, { type: 'bot', content: 'Thanks for stopping by!' }]);
      setIsComplete(true);
      return;
    }
      ...
      try {
        ...
        if (res.ok && result.success) {
          ...
          setMessages(m => [
            ...
            { 
              type: 'bot', 
              content: `❌ Submission failed: ${result.error || result.message}` 
            },
          ]);
        }
      } catch (e: unknown) {
  ...
  const handleSubmitDetails = () => {
    ...
    if (q.type === 'input' && q.fields) {
      const missing = q.fields.filter(f => f.required && !userDetails[f.name]);
      if (missing.length) {
        setMessages(m => [
          ...m,
          { 
        ...
        >
          ...
          <div className="px-4 py-3 bg-white border-t border-gray-200">
            {!isComplete && !isSubmitting ? (
              ...
              <div className="text-center text-gray-500 text-sm">Submitting…</div>
            ) : (
              <button
                onClick={restart}
